...
/************ SEARCH ***********/
$con = connectTo();
$exists = $con->query("select * from `online-test2`.`registration` where email = '$email'");
if($exists && $con->affected_rows) {
  $con->close();
  respond("error","exists");
} 
/*********** INSERT ************/
$addTeacher = $con->query("insert into `online-test2`.`registration` (`name`, `email`, `phone`, `password`) values ('$name', '$email', '$phone', '$pass')");
